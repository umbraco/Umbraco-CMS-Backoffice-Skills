...
- **Mocked tests** - Playwright against the Umbraco **client dev server** (MSW). Needs
  `UMBRACO_CLIENT_PATH` pointing at a built v17 `Umbraco.Web.UI.Client`.
...
| `UMBRACO_CLIENT_PATH` | - | Path to a built v17 `Umbraco.Web.UI.Client` — required for the mocked-backoffice suites |
...
v17 client; without it those suites are skipped.)
